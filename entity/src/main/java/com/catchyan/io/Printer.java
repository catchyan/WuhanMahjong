...
public class Printer {
    ...
    public static void printPai(Tile tile, Style style){
        try(InputStream inputStream = Objects.requireNonNull(Printer.class.getClassLoader().getResource(style.getFileName())).openStream()){
            ...
            inputStream.skip(style.getTypeOffset() * tile.getSuit().getOffset() + (tile.getPoint() - 1) * pointOffset);
            byte[] bytes = new byte[new Long(pointOffset).intValue()];
            inputStream.read(bytes);
            String s = new String(bytes);
            System.out.print(s);
        } catch (IOException e) {
            Printer.print("exception Tile" + tile.getSuit() + tile.getPoint());
